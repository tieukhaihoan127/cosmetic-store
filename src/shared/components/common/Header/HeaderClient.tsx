...
const HeaderClient = () => {
    ...
    const { hoveredCategory, handleHoverCategory, handleLeaveCategory, handleOpenLoginDialog, handleOpenWishlistDrawer, handleOpenCartDrawer } = useHeader();
                ...
                <div className="w-[90%] max-w-[90%] mx-auto flex item-center justify-between">
                    ...
                    <div className="flex flex-1 items-center justify-end max-md:flex-none">
                        ...
                        <div className="flex items-center gap-3 max-md:gap-4 max-sm:gap-3">
                            <div onClick={handleOpenLoginDialog} className="flex justify-center items-center hover:cursor-pointer max-md:hidden">
                                <AccountCircleOutlinedIcon sx={{ fontSize: {sm:25, md: 28, lg:30} }}/>
                                <div className="font-bold text-[12px] text-xs ml-2 text-[var(--text_primary)] [@media(max-width:1144px)]:hidden">
                                    Đăng nhập
                                </div>
                            </div>
                            <FavoriteBorderOutlinedIcon onClick={handleOpenWishlistDrawer} sx={{ fontSize: {sm:25, md: 28, lg:30}, textAlign: 'right', cursor: 'pointer' }}/>
                            <ShoppingCartOutlinedIcon onClick={handleOpenCartDrawer} sx={{ fontSize: {sm:25, md: 28, lg:30}, cursor: 'pointer' }}/>
                        </div>  
                    </div>
                </div>
            </div>
