  ...
  return (
    <div className='flex items-center  header-carousel'>
      ...
      <Swiper modules={[Navigation]} navigation={{ prevEl: ".custom-prev", nextEl: ".custom-next" }} slidesPerView="auto" slidesPerGroup={10} spaceBetween={20} watchOverflow={true} centeredSlides={false} onSwiper={(swiper) => {
        ...
        <SwiperSlide className='w-auto'>
          <HeaderCategoryButton content='Khuyến mãi hot' to='/collections/khuyen-mai-hot' />
        </SwiperSlide>
        <SwiperSlide className='w-auto'>
          <HeaderCategoryButton content='Sản phẩm cao cấp' to='/' />
        </SwiperSlide>
        <SwiperSlide className='w-auto'>
          <HeaderCategoryButton content='Trang điểm' to='/categories/trang-diem' onHover={() => onHoverCategory('Trang điểm')} onLeave={onLeaveCategory}/>
        </SwiperSlide>
        <SwiperSlide className='w-auto'>
